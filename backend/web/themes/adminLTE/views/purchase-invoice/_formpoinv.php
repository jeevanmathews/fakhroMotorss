...
<?php $form = AutoForm::begin(["id" => "purchase-invoice-".time().(($model1->isNewRecord)?"createpoinv":"update")."-form"]); ?>
<div class="box-body">
    <div class="row">
        <div class="col-md-12">
           <?= $form->field($model1,'po_id')->hiddenInput(['value'=>$model->id])->label(false) ?>
            <div class="row">
                <div class="col-md-6"> 
                   ...
                   <div class="form-group field-deliveryorder-do_number">
                      <div class="input-group">
                        <div class="input-group-addon">PO Number</div>
                        <?= Html::textInput('ponum', $model->prefix->prefix.' '.$model->po_number, ['class'=>'form-control','disabled'=>true]); ?>
                      </div>
                    </div>
                    ...
                    <?= $form->field($model1, 'supplier_id', ['inputOptions' => ["class" => "form-control select2",'disabled'=>true]])->dropDownList(ArrayHelper::map(Supplier::find()->where(["status" => 1])->all(), 'id', 'name'), ["prompt" => "Select Supplier",'value'=>(isset($model->supplier_id)?$model->supplier_id:'')]) ?>                                     
                        ...
                        <tbody class="item_table">
                            ...
                            <tr class="item_row" rid="1">
                                ...
                                <td> <?= $form->field($modelpr, 'grn_quantity[]')->textInput(['value'=>(($req->remaining_quantity!=0)?$req->remaining_quantity:$req->quantity),'class'=>'form-control remaining_qty disabled'])->label(false) ?> </td>
                                 <td> <?= Html::activeTextInput($modelpr,'grn_quantity[]',['type'=>'hidden','class'=>'','value'=>(($req->remaining_quantity!=0)?$req->remaining_quantity:$req->quantity)])?>
...
$('body').on('change','#purchaseinvoice-po_id',function(){
 ...
 if(po_id!='' && po_id!='undefined'){
    // window.location.href='<?php echo Yii::$app->getUrlManager()->createUrl("purchase-invoice/createpoinv"). "&id="?>'+po_id;
    $.ajaxSetup({async: false}); 
          $.ajax({
          url: '<?php echo Yii::$app->getUrlManager()->createUrl("purchase-invoice/createpoinv")?>',//"'+po_id+'
          aSync: false,
          data:{'id':po_id},
          dataType: "html",
