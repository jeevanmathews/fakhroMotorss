...
<?php $form = AutoForm::begin(['id'=>'soform']); ?>
<div class="box-body">
  <div class="row">
   <?php //if($model->pr_id): ?>
   <div class="mb-20">
    ...
    <div class="col-md-6"> 
      <?= $form->field($model,'qtn_id', ['inputOptions' => ["class" => "select_pr form-control select2"]])->dropDownList(ArrayHelper::map(Qutation::find()->where(["status" => 1])->all(), 'id', 'qtn_number'), ["prompt" => "Select Qutation"]) ?>
    </div>
    <?php if($type=='create'): ?>
    <div class="col-md-6 "> 
    ...
    <tbody class="item_table">
      ...
      <tr class="item_row" rid="1">
        ...
        <td><?= $form->field($model1, 'price[]')->textInput(['class'=>'form-control price'])->label(false) ?>
          <input type="" class="total_price" name="SalesOrderItems[total_price][]">
        </td>
        <?php //if($vat_format=="inclusive") :?>
        <td>
      ...
      <tr class="item_row" rid="1">
        ...
        <td><?= $form->field($req, 'price[]')->textInput(['value'=>$req->price,'class'=>'form-control price'])->label(false) ?>
          <input type="" class="total_price" value="<?=$req->total_price?>" name="SalesOrderItems[total_price][]">
        </td>
        <?php if($vat_format=="inclusive") :?>
        <td>
          <div id="" role="radiogroup" aria-invalid="true">
...
$('body').on('click','.btn_select_pr',function(){
  var qtn_id=$("#salesorder-qtn_id").val();
  // console.log(pr_id);
  // "<?php echo $pr_id     = "+pr_id+" ?>";
  // console.log();
  // console.log("<?php echo Yii::$app->getUrlManager()->createUrl(['purchaseorder/createpo','id'=>$pr_id]);?>");
  if(qtn_id!='' && qtn_id!='undefined'){
    window.location.href='<?php echo Yii::$app->getUrlManager()->createUrl("sales-order/createso"). "&id="?>'+qtn_id;
  }
});
